...
monitorBounds = {"top": 305, "left": 655, "width": 300, "height": 300}
...
##    originalImg = np.array(ImageGrab.grab(bbox=(655, 305, 955, 605)), dtype=np.int32)[...,::-1]
##    originalImg = np.array(ImageGrab.grab(bbox=(655, 305, 955, 605)))[...,::-1].copy()
##    originalImg2 = 
##    ptx1 = (mask_cx - 30, mask_cy)
##    ptx2 = (mask_cx + 30, mask_cy)
##    pty1 = (mask_cx, mask_cy - 30)
##    pty2 = (mask_cx, mask_cy + 30)
##    cv2.line(originalImg, ptx1, ptx2, (0,0,255), 1)
##    cv2.line(originalImg, pty1, pty2, (0,0,255), 1)
##    if drawControl: cv2.imshow("PIL input", originalImg)
    img_time = time.time()
    with mss.mss() as sct:
        originalImg = np.array(sct.grab(monitorBounds))[:,:,:3]#.copy()
##        cv2.line(originalImg2, ptx1, ptx2, (0,0,255), 1)
##        cv2.line(originalImg2, pty1, pty2, (0,0,255), 1)
##        if drawControl: cv2.imshow("MSS input", originalImg2)
    print("time to grab:", time.time()-img_time)
##    print(originalImg.shape, originalImg2.shape)
...
##    cv2.imshow("prog", dst)     #thresholded image
    ...
    if drawControl: houghIm = cv2.cvtColor(dst.copy(), cv2.COLOR_GRAY2RGB)
    if drawControl: houghFiltered = houghIm.copy()
    houghRes1 = []
...
##                    print("near origin", slope, rho, theta, math.degrees(theta))
                    if drawControl: cv2.line(houghIm, pt1, pt2, (0,255,0), 1, cv2.LINE_AA)
                    houghRes1.append([theta, rho])
            elif theta == 0 and abs(rho - origin_x) < merge_eps:
##                print("vertical near origin")
                if drawControl: cv2.line(houghIm, pt1, pt2, (255,0,0), 1, cv2.LINE_AA)
                houghRes1.append([math.pi, rho])
        if drawControl: cv2.circle(houghIm, (origin_x, origin_y), merge_eps, (0,0,255), 1)
    else:
##        print("no mining detected")
##                time.sleep(1)
        return
    if drawControl: cv2.imshow("hough", houghIm)
    if len(houghRes1) == 0:
        return
    houghRes1.sort()

    print("time for hough analysis:", time.time() - hough_time)
...
##        print("bad classification")
    ...
    print("time for target analysis:", time.time() - target_time)
...
##    print("time to grab:", time.time()-img_time)
    if drawControl: cv2.imshow("orig", originalImg)

    #find current position
    ''''''
    if drawControl: posImg = originalImg.copy()
    prevMiss = 0
    pos = 180
    for deg in range(0, 180, 5):
        ...
        print("and time:", subset_time - mask_start)
...
##            cv2.line(posImg, (origin_x, origin_y), (x,y), (255,0,0), 1, cv2.LINE_AA)
    if drawControl: cv2.imshow("pos img", posImg)
    print("time for progress analysis:", time.time() - progress_time)
    print("time for masking:", mask_time)
    print("time for progress:", prog_time)
    ''''''

    if len(center) != 0:
        print("center:", (360 + math.degrees(center[0])) % 360, "pos:", pos)
        if drawControl:
            radius = 100
    ...
    ##        cv2.line(houghFiltered, (origin_x, origin_y), (x,y), (255,0,0), 1, cv2.LINE_AA)
        ...
        mouse_offset = 8
        if abs(pos + mouse_offset - (360 + math.degrees(center[0])) % 360) < 8:
            #trigger mouse up
            posX, posY = getMousePos()
            windll.user32.mouse_event(0x0004, posX, posY, 0, 0)
